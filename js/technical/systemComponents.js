var systemComponents = {
	...
	'overlay-head': {
		template: `			
		<div class="overlayThing" style="padding-bottom:7px; width: 20%; z-index: 1000; position: relative">
		...
		<h2  class="rainbowThing" id="points">{{format(player.points)}}</h2>
		<span v-if="player.points.lt('1e1e6')"  class="rainbowThing"> {{modInfo.pointsName}}</span>
		<br>
		<span v-if="canGenPoints()"  class="rainbowThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}}/sec)</span>
