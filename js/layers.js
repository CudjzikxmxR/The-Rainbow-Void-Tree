...
addLayer("g", {
    ...
    milestones: {
        17: {
            ...
            effectDescription() {
                if (hasUpgrade('p', 31)) {
                    return "You can feed Axe Cat, completely disabling Amoeba gain and causing Catfood to spawn around the screen, boosting your Rainbow and Cherry gain for each Catfood clicked. The cap scales based on your clicking power. Both the cap and the boost per click scales based on your coinflip-related Cherry multiplier.<br><b>Currently "+format(player.AxeCatMult)+"x and ^"+format(Math.log(player.AxeCatMult)/100)+". (Capped at "+format((1+Math.log(getClickPower())/Math.log(3.07)*10*player.CoinflipMult/200))+"x and "+format(Math.log2((1+Math.log(getClickPower())/Math.log(3.07)*10*player.CoinflipMult/200))/100)+"</b>"
                } else {
